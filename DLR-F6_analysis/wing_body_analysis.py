...
parser.add_argument("--output", type=str, default="output_medium") #在当前文件夹下创建“output”文件夹
parser.add_argument("--gridFile", type=str, default="wb_medium_scale.cgns")    #网格文件，测试的时候用粗网格，是否需要网格无关性验证?
...
# Add features
CFDSolver.addLiftDistribution(150, "y") #用沿展长方向的150~250个点拟合升力、阻力等参数的分布曲线，所得.dat文件中共有26个变量，每个变量150个点
CFDSolver.addSlices("y", [-0.0878475,-0.13997035,-0.19385015,-0.22079005,-0.24070215,-0.3010241,-0.3736447,-0.49604555])#沿展长（y）方向创建10个等距截面，将翼型坐标和 cp 分布写入文本文件

# rst Create AeroProblem
ap = AeroProblem(name="wing_body_medium", mach=0.75, reynolds=3.0e6, reynoldsLength=0.1412, T=300, alpha=0.49, areaRef=0.07265, chordRef=0.1412, evalFuncs=["cl", "cd"]) #采用半模计算，故参考面积0.1453/2 m²
# rst Run ADflow
